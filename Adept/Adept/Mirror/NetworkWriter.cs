...
namespace TelepathyServerTest
{
	...
	public class NetworkWriter
	{
		...
		public void WriteBytesAndSize(byte[] buffer, int offset, int count)
		{
			...
			{
				System.Diagnostics.Debug.WriteLine("NetworkWriter WriteBytesAndSize: size is too large (" + count + ") bytes. The maximum buffer size is " + ushort.MaxValue + " bytes.");
				return;
			}

			if (count < 0)
			{
				System.Diagnostics.Debug.WriteLine("NetworkWriter WriteBytesAndSize: size " + count + " cannot be negative");
				return;
			}
